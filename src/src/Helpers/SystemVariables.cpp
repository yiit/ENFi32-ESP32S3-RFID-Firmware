...
// Global variable for RFID Card ID
static String _lastCardID = "";
static String _lastCardTimestamp = "";

// Global variable for Server Connection Status  
static bool _serverConnectionStatus = false;
...
String SystemVariables::getSystemVariable(SystemVariables::Enum enumval) {
  ...
  switch (enumval)
  {
    ...
    case CARDID:            return SystemVariables::getCardID();                    // RFID Card ID from USB HID
    case CARDTIMESTAMP:     return SystemVariables::getCardTimestamp();            // RFID Card read timestamp
    ...
    case SERVERSTATUS:      return SystemVariables::getServerStatus();              // PDKS Server connection status
...
SystemVariables::Enum SystemVariables::startIndex_beginWith(const char* beginchar)
{
  ...
  switch (ch)
  {
    ...
    case 'c': return Enum::CARDID;        // RFID Card ID - ilk sırada olmalı
    ...
    case 't': return Enum::CARDTIMESTAMP;     // %timestamp% - RFID kart okuma zamanı
...
const __FlashStringHelper * SystemVariables::toFlashString(SystemVariables::Enum enumval)
{
  switch (enumval) {
    ...
    case Enum::CARDID:             return F("cardid");
    case Enum::CARDTIMESTAMP:      return F("timestamp");
    ...
    case Enum::SERVERSTATUS:       return F("serverstatus");
...
// RFID Card ID methods
String SystemVariables::getCardID() {
  return _lastCardID;
}

void SystemVariables::setCardID(const String& cardid) {
  _lastCardID = cardid;
}

String SystemVariables::getCardTimestamp() {
  return _lastCardTimestamp;
}

void SystemVariables::setCardTimestamp(const String& timestamp) {
  _lastCardTimestamp = timestamp;
}

// Server connection status methods  
String SystemVariables::getServerStatus() {
  return _serverConnectionStatus ? "1" : "0";
}
