...
/*void P020_Task::serialTime() {
  if (nullptr != ser2netSerial) {
      String time = "mes%systm_hm_0%   \r\n";
      parseSystemVariables(time, false);
      ser2netSerial->write(time.c_str(), time.length());
  }
}*/

// rules1.txt içinde "#SERIALTIME=..." satırını bulup 'out'a yazar.
static bool loadTaggedLine_LFS(const char* path, const char* tag, String& out) {
  // LittleFS zaten init'liyse ikinci kez begin() harmless ama dilersen
  // bunu setup'ta bir kere çağırıp burada kontrolü kaldırabilirsin.
  if (!LittleFS.begin()) {
    // Serial.println(F("[LFS] begin() failed"));
    return false;
  }

  if (!LittleFS.exists(path)) {
    // Serial.println(F("[LFS] rules dosyası yok"));
    return false;
  }
  ...
  const String lookFor = String(tag) + "=";   // "#SERIALTIME="
  while (f.available()) {
    String line = f.readStringUntil('\n');
    line.trim();                // baş/son boşluk ve \r temizle
    if (line.startsWith(lookFor)) {
      out = line.substring(lookFor.length()); // "tag=" kısmını kes
      f.close();
      return true;
    }
  }
...
// İstersen hafif bir cache: (dosyayı her çağrıda okumayı azaltır)
static String g_serialTimeCached;
static bool   g_serialTimeLoaded = false;

void P020_Task::serialTime() {
  if (ser2netSerial == nullptr) return;

  String time;

  // 1) Önbellekte varsa kullan; yoksa LFS’ten yükle
  if (g_serialTimeLoaded) {
    time = g_serialTimeCached;
  } else {
    if (loadTaggedLine_LFS("/rules1.txt", "#SERIALTIME", time)) {
      g_serialTimeCached = time;
      g_serialTimeLoaded = true;
    } else {
      // 2) fallback (dosyada yoksa)
      time = F("mes%systm_hm_0%   \r\n");
    }
  }

  // 3) %systm_*% vb. değişkenleri çöz
  parseSystemVariables(time, false);
